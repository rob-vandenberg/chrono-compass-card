          ...
          <svg class="compass-needle"
            ...
            ${e.image_show&&m?svg`
              <image
                href="${m}"
                x="${s}" y="${a}"
                width="${n-s}" height="${r-a}"
                preserveAspectRatio="xMidYMid slice"
                clip-path="url(#${p})"
                transform="translate(${(n+s)/2}, ${(r+a)/2}) rotate(${e.image_rotate}) scale(${e.image_scale/100}) translate(${e.image_x}, ${e.image_y}) translate(${-(n+s)/2}, ${-(r+a)/2})"
              />
            `:svg`
              <path d="${l.path}" fill="url(#${_})" />
            `}
          </svg>
        </div>
      `};return html`
        ...
        <div class="compass-container">
          <div class="compass-layer">
            ${e.background_image_show&&this._backgroundImageUrl?html`
              ...
              />
            `:""}
            ${s(l[0],this._field1Value)}
            ${s(l[1],this._field2Value)}
            ${s(l[2],this._field3Value)}
          </div>
